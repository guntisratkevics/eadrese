...
if ($messageId === '') {
    fwrite(STDERR, "DIV_MESSAGE_ID is required.\n");
    exit(2);
}
...
$envelopeXml = is_array($body) ? ($body['EnvelopeXml'] ?? null) : null;
$recipients = is_array($body) ? ($body['Recipients'] ?? null) : null;
$confirmationName = is_array($body) ? ($body['ConfirmationName'] ?? null) : null;
